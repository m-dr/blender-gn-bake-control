...
def draw_gn_bake_ui(layout, context):
    ...
    if state:
        row_tools.prop(state, "show_disconnected", text="Show Disconnected / Stale", icon='HIDE_OFF' if state.show_disconnected else 'HIDE_ON')
    ...
    for entry in mod_data:
        ...
        count_label = f"({conn_count} Active" + (f", {dis_count} Disconnected)" if dis_count > 0 else ")")
        ...
        for b in bakes:
            ...
            # Grey out if disconnected or muted
            ...
            # 2. Compact right arrow navigate & frame node button
            if b.get("node_name"):
            ...
            # 3. Node display name / path + Simulation / Type icon
